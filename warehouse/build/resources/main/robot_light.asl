...
/*******************************************************************************
 * AGENTE: Robot ligero
 * DESCRIPCIÓN: Robot ágil diseñado para manejar cargas ligeras en el almacén.
 * CAPACIDAD: 10 kg 
 * TAMAÑO MÁXIMO: 1×1
 * VELOCIDAD: Alta (3)
 * Posicion de inicio(lightInit): (1,3)
 ********************************************************************************
 CREENCIA INICIALES:
...
+!start : true <-
    ...
    .wait(500);
    .send(scheduler, tell,robot_available(robot_light)).
...
+!work : state(idle) <-
    .send(scheduler, untell,robot_available(X));
    assignTask. //decirle al entorno que al robot le asigne siguiente tarea disponible.
+!work : not state(idle) <-
...
// no se utilizara en las versiones finales, solo para pruebas de movimiento
